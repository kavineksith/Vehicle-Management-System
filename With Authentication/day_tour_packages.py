...
class PerDayPackagesApp:
    ...
    def insert_package(self, package_data):
        try:
            ...
            cursor.execute('''INSERT INTO PerDayTourPackages (PackageID, PackageType, PackageBasedVehicleType, MaxKMLimit, MaxHourLimit, PackagePrice)
                              VALUES (?, ?, ?, ?, ?, ?)''', package_data)
    ...
    def update_package(self, package_data):
        try:
            cursor = self.db_connection.cursor()
            cursor.execute('''UPDATE PerDayTourPackages SET PackageType=?, PackageBasedVehicleType=?, MaxKMLimit=?, MaxHourLimit=?, PackagePrice=? 
                              WHERE PackageID=?''', package_data[1:] + [package_data[0]])
            self.db_connection.commit()
    ...
    def delete_package(self, package_id):
        try:
            cursor = self.db_connection.cursor()
            cursor.execute('''DELETE FROM PerDayTourPackages WHERE PackageID=?''', (package_id,))
            self.db_connection.commit()
    ...
    def search_package(self, package_id):
        try:
            cursor = self.db_connection.cursor()
            cursor.execute('''SELECT * FROM PerDayTourPackages WHERE PackageID=?''', (package_id,))
    ...
    def filter_package(self, package_id):
        try:
            cursor = self.db_connection.cursor()
            cursor.execute('''SELECT * FROM PerDayTourPackages WHERE PackageID=?''', (package_id,))
        ...
        while True:
            try:
                ...
                if choice == '1':
                    ...
                    package_id = input("Enter Package ID to filter: ")
                    self.filter_package(package_id)
                elif choice == '6':
                    self.view_packages()
                elif choice == '7':
    ...
    def get_package_data_from_input(self):
        ...
        max_km_limit = self.get_valid_max_km_limit()
        max_hour_limit = self.get_valid_max_hour_limit()
        package_price = self.get_valid_package_price()
        return (package_id, package_type, package_based_vehicle_type, max_km_limit, max_hour_limit, package_price)
    ...
    def get_valid_max_hour_limit(self):
        while True:
            try:
                max_hour_limit = input("Enter Max Hour Limit: ")
                if re.match(r'^\d+$', max_hour_limit):
                    return int(max_hour_limit)
                else:
                    print("Invalid max hour limit! Please enter a valid integer.")
            except KeyboardInterrupt:
                print("Process interrupted by the user.")
                sys.exit(1)
            except Exception as e:
